...
Router::Router() {
    not_found_handler_ = [](HttpContext& context, const net::TcpConnection::Ptr& conn, const Next& next) {
...
void Router::Route(HttpContext& context, const net::TcpConnection::Ptr& conn) {
    std::string key = context.request.GetMethod() + ":" + context.request.GetPath();
    
    auto it = routes_.find(key);
    const auto& chain = (it != routes_.end()) ? it->second : std::vector<Middleware>{not_found_handler_};
